...
import { addWallet, deleteWallet, getWallets } from '../db/wallets';
import {
    colors,
    FONT_FAMILY,
    FONT_FAMILY_BOLD,
...
export default function Wallets() {
    ...
    const handleAddWallet = () => {
        Alert.prompt?.('محفظة جديدة', 'أدخل اسم المحفظة', async (name) => {
            if (name && name.trim()) {
                await addWallet(name.trim(), 0);
                load();
            }
        });
    };
    ...
    return (
        ...
        >
            <View style={styles.header}>
                <Text style={styles.headerTitle}>المحافظ</Text>
                <Avatar size={36} />
            </View>
